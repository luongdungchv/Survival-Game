...
namespace Dacodelaac.Utils
{
    public static class MonoBehaviorExtension
    {
        ...
        public static bool CanStartCoroutine(this MonoBehaviour mono)
        {
        ...
        public static Coroutine WaitForEndOfFrame(this MonoBehaviour mono, Action callback)
        {
            return mono.StartCoroutine(WaitForEndOfFrame(callback));
        }

        static IEnumerator WaitForEndOfFrame(Action callback)
        {
            yield return new WaitForEndOfFrame();
            callback?.Invoke();
        }
        
        public static T GetAndCacheComponent<T>(this GameObject mono, ref T cache) where T : Component
        {
            return cache ? cache : (cache = mono.GetComponent<T>());
        }
        
        public static T GetAndCacheComponentInChildren<T>(this GameObject mono, ref T cache, bool includeInactive = false)
        {
            return cache ??= mono.GetComponentInChildren<T>(includeInactive);
        }
        
        public static T[] GetAndCacheComponents<T>(this GameObject mono, ref T[] cache) where T : Component
        {
            return cache ??= mono.GetComponents<T>();
        }
        
        public static T[] GetAndCacheComponentsInChildren<T>(this GameObject mono, ref T[] cache, bool includeInactive = false)
        {
            return cache ??= mono.GetComponentsInChildren<T>(includeInactive);
        }

        public static T GetAndCacheComponentInParent<T>(this GameObject mono, ref T cache)
        {
    ...
    public static class TransformExtensions
    {
        ...
        public static void CopyLocal(this Transform transform, Transform other, bool position, bool rotation, bool scale)
        {
            if (position) transform.localPosition = other.transform.localPosition;
            if (rotation) transform.localRotation = other.transform.localRotation;
            if (scale) transform.localScale = other.localScale;
        }

        public static void ResetLocal(this Transform transform, bool scale = true)
        {
        ...
        {
            return obj == null || (obj is UnityEngine.Object uo && uo == null);
        }
        ...
        [MenuItem("CONTEXT/Transform/ResetParentPos")]
        public static void ResetParentPos(MenuCommand command)
        {
            var tfs = Selection.gameObjects.Select(g => g.transform).ToArray();
            if (tfs.Length > 0)
            {
                for (var i = 0; i < tfs.Length; i++)
                {
                    var offset = tfs[i].position;
                    tfs[i].position = Vector3.zero;
                    EditorUtility.SetDirty(tfs[i]);

                    for (var j = 0; j < tfs[i].childCount; j++)
                    {
                        var child = tfs[i].GetChild(j);
